...
import 'package:vnedu/shared/other_new_board.dart';
import 'package:vnedu/shared/profile_menu.dart';
import 'package:vnedu/shared/study_corner_details.dart';

class AnnouncementDetailScreen extends StatefulWidget {
  const AnnouncementDetailScreen({super.key, required this.isLogged, required this.newHots});
  final bool isLogged;
  final List<NewHots> newHots;
...
class _AnnouncementDetailScreenState extends State<AnnouncementDetailScreen> {
  ...
  List<Widget> getListNewHosts(AppBarNotifier appBarNotifier)  {
    List<Widget> ans = [];
    for(int i = 0; i < widget.newHots.length; i++) {
      ans.add(
        OtherNewBoard(
            newHot: widget.newHots[i],
            onTapShowDetails: () async {
              appBarNotifier.setAnnouncementDetail(
                await getAnnouncementDetail(widget.newHots[i].id)
              );
            }
        )
      );
      ans.add(const SizedBox(height: 20,));
    }
    return ans;
  }
                  ...
                  SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        AnnouncementDetailPage(announcementDetail: appbarNotifier.announcementDetail),
                        ...
                        Column(
                          children: getListNewHosts(appbarNotifier)
                        )
                      ],
                    ),
                  ),
